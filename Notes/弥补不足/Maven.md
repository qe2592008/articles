...
validate->complie->test->package->verify->install->deploy
    ...
    - depoly：拷贝最终的工程包到远程仓库中，以共享给其他开发人员和工程
先后顺序，执行其中某个命令，其前面的命令会先执行，后面的命令不执行
2. 快照SNAPSHOT
使用快照建包，可以保证每次打包时自动获取最新的版本
> 公开版本中不适合使用快照版本，内部可以使用，还可以享用其实时更新的功能
公开版本已经发布不得再修改，所以已发布就固定内容不变了，用不着实时更新，因为永久不变，如果有BUG，需要修复后新发一版本
3. 配置文件
pom>个人setting>公共setting
4. 仓库
本地仓库>中央仓库>远程仓库
5. 插件
6. 依赖关系
scope   
    - compile(默认)：编译时需要用到该jar包，例如：commons-logging
    - test：编译Test时需要用到该jar包，例如：junit
