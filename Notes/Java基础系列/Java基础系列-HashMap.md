...
针对第一种情况，真的好简单，待删节点即为叶子节点，直接删除即可；<br/>
...
> - 在Jdk1.8以前的版本中在高并发的情况下，HashMap数组扩容的时候可能会出现死循环，这是因为两个线程同时进行扩容和元素迁移导致出现了循环链表，Jdk1.8中已经修复这一Bug，采用的是将头插法改为尾插法，但是HashMap仍然是线程不安全的集合，多线程环境中最好使用ConcurrentHashMap。
